...
$configurations =
@(
    @($templateConfiguration[0], $releaseX64),
    @($templateConfiguration[1], $release),
    @($templateConfiguration[2], $releaseIPhone),
    @($templateConfiguration[3], $releaseIPhoneSimulator),
    @($templateConfiguration[4], $release)
)

# Default
dotnet new unoapp -n UnoAppAll
& $msbuild $debug UnoAppAll\UnoAppAll.sln
& $msbuild $release UnoAppAll\UnoAppAll.sln
...
# VS Code
dotnet new unoapp -n UnoAppVsCode (Get-Configuration -wasm 1 -wasmVsCode 1)
